...
# On this file Channel 1 has 180 problem
dbFile <- 'E:/Hiceas/Database/1706_pg11511_sb_35_20170914.sqlite3'
pamDb <- loadDB(dbFile)
pamData <- formatStation(pamDb)
myData <- loadGpsDifar(dbFile) %>% 
      mutate(RealBearing = geosphere::bearing(cbind(BuoyLongitude, BuoyLatitude), cbind(BoatLongitude, BoatLatitude)) %% 360,
